...
function moveGhost(ghost) {
  ...
  const path = (ghost.smart === true ) ? findSmartPath(ghost, pacmanObj) : findNormalPath()
  
  if (path && path.length > 1) {
    const [nextX, nextY] = path[1];
    const nextBlock = document.querySelector(`.square[data-x="${nextX}"][data-y="${nextY}"]`);
  ...
  while (queue.length > 0) {
    ...
    for (const [dx, dy] of directions) {
      ...
      if (!visited.has(newKey)) {
        const nextBlock = document.querySelector(`.square[data-x="${newX}"][data-y="${newY}"]`);
        if (nextBlock && !nextBlock.classList.contains('border')) {
...
function handleCollision() {
  ...
  const ghost = ghosts[ghosts.findIndex(g => g.x === pacmanObj.x && g.y === pacmanObj.y)]
  ghost.resetPosition()
